...
|------|----------|-----------|------------------|
| Domain imports application context types | `artifacts/mobile/domain/references/EntityReferencePolicy.ts` previously imported `Task`, `CalendarEvent`, `Note`, `Person`, `Transaction` from `context/*`; `ReferenceCleanupOrchestrator.ts` did the same and has been removed | Domain must not import React contexts | T-014 (completed): refactored to feature-neutral DTOs and removed dead orchestrator |
| Adapter imports context types and AsyncStorage | `artifacts/mobile/domain/references/ReferenceCleanupService.ts` previously imported from `context/*` and `AsyncStorage` directly | Infrastructure adapter should depend only on domain DTOs and storage port | T-014 (completed): introduced `ReferenceCollectionStore` port and `createAsyncStorageReferenceCollectionStore` adapter |

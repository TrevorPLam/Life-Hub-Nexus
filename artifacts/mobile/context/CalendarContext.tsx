...
export function CalendarProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then(raw => {
      if (raw) {
        try { setEvents(JSON.parse(raw)); } catch { setEvents(SEED_EVENTS); }
      } else {
  ...
  const addEvent = useCallback((event: Omit<CalendarEvent, 'id' | 'createdAt'>): CalendarEvent => {
    ...
    setEvents(prev => { const u = [...prev, newEvent]; AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(u)); return u; });
    return newEvent;
  }, []);

  const updateEvent = useCallback((id: string, updates: Partial<CalendarEvent>) => {
    setEvents(prev => { const u = prev.map(e => e.id === id ? { ...e, ...updates } : e); AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(u)); return u; });
  }, []);

  const deleteEvent = useCallback((id: string) => {
    setEvents(prev => { const u = prev.filter(e => e.id !== id); AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(u)); return u; });
  }, []);
  ...
  const getEventsForDate = useCallback((date: Date) => {
    ...
    return events.filter(e => new Date(e.startDate).toDateString() === dateStr).sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime());
  }, [events]);

  const getUpcomingEvents = useCallback((count = 5) => {
    const now = new Date();
    return events.filter(e => new Date(e.startDate) >= now).sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime()).slice(0, count);
  }, [events]);

  return (
    <CalendarContext.Provider value={{ events, loading, addEvent, updateEvent, deleteEvent, getEvent, getEventsForDate, getUpcomingEvents }}>
      {children}
